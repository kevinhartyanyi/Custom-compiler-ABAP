// Generated by Bisonc++ V5.02.00 on Sun, 26 Apr 2020 21:33:00 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {303, 4}, // 1: start (PROGRAM) ->  PROGRAM VAR END declaration
     {304, 1}, // 2: declaration ->  body
     {304, 4}, // 3: declaration (DATA) ->  DATA DDOT vardeclaration body
     {305, 5}, // 4: vardeclaration (VAR) ->  VAR TYPE INT SEPARATOR vardeclaration
     {305, 5}, // 5: vardeclaration (VAR) ->  VAR TYPE BOOL SEPARATOR vardeclaration
     {305, 4}, // 6: vardeclaration (VAR) ->  VAR TYPE INT END
     {305, 4}, // 7: vardeclaration (VAR) ->  VAR TYPE BOOL END
     {306, 0}, // 8: body ->  <empty>
     {306, 1}, // 9: body ->  sequence
     {307, 2}, // 10: sequence ->  statement sequence
     {307, 1}, // 11: sequence ->  statement
     {308, 5}, // 12: statement (MOVE) ->  MOVE expression TO VAR END
     {308, 2}, // 13: statement (END) ->  arithmetic END
     {308, 3}, // 14: statement (WRITE) ->  WRITE expression END
     {308, 6}, // 15: statement (WHILE) ->  WHILE logic END sequence ENDWHILE END
     {308, 4}, // 16: statement (READ) ->  READ TO VAR END
     {308, 6}, // 17: statement (IF) ->  IF logic END sequence ENDIF END
     {308, 9}, // 18: statement (IF) ->  IF logic END sequence ELSE END sequence ENDIF END
     {309, 1}, // 19: logic ->  expression
     {309, 3}, // 20: logic (EQUAL) ->  logic EQUAL logic
     {309, 3}, // 21: logic (OR) ->  logic OR logic
     {309, 3}, // 22: logic (AND) ->  logic AND logic
     {309, 2}, // 23: logic (NOT) ->  NOT logic
     {309, 4}, // 24: logic (SMALLER) ->  logic SMALLER EQUAL logic
     {309, 4}, // 25: logic (BIGGER) ->  logic BIGGER EQUAL logic
     {309, 3}, // 26: logic (SMALLER) ->  logic SMALLER logic
     {309, 3}, // 27: logic (BIGGER) ->  logic BIGGER logic
     {309, 3}, // 28: logic (OPEN) ->  OPEN logic CLOSE
     {310, 1}, // 29: expression (VAR) ->  VAR
     {310, 1}, // 30: expression (NUM) ->  NUM
     {310, 1}, // 31: expression (TRUE) ->  TRUE
     {310, 1}, // 32: expression (FALSE) ->  FALSE
     {311, 4}, // 33: arithmetic (SUBTRACT) ->  SUBTRACT expression FROM VAR
     {311, 4}, // 34: arithmetic (ADD) ->  ADD expression TO VAR
     {311, 4}, // 35: arithmetic (MULTIPLY) ->  MULTIPLY VAR BY expression
     {311, 4}, // 36: arithmetic (DIVIDE) ->  DIVIDE VAR BY expression
     {311, 6}, // 37: arithmetic (MOD) ->  MOD VAR BY expression TO VAR
     {312, 1}, // 38: start_$ ->  start
};
